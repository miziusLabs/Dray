...
- **The mapper is partial.** `assistant`, `user`, `result`, `system/init`, tasks, `background_tasks_changed`, and the stream frames are wired; `status`, hooks, `post_turn_summary`, `task_updated`, and `rate_limit_event` still fall through to `None`. `turn_id` is always `None` — deliberately: Claude Code has no turn identifier on the wire, the UI groups a transcript by user message, and a promptless agent-opened `init` (see below) means minting our own ids would split what the reader sees as one exchange.
