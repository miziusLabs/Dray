...
**The card renders outside the turn stack**, below the transcript beside the background-task and compacting indicators, from `buildTranscript`'s `pendingPermissions`. One place serves both threads: a subagent's tool call is filed into the panel, and a main-thread one sits in a turn that collapses once it closes, so *neither* can be relied on to be next to the question. That is also why the card carries the command itself — nothing above it is guaranteed to be on screen. It follows that `permission_requested` is **not** in `RENDERS`: it draws no row inside a turn, so counting it as one would miscount the collapse. And an open request suppresses the thinking indicator, like a compaction does — the turn genuinely draws nothing, but the agent is waiting on the reader rather than working.

**A permission request is never written to the log.** It joins deltas and usage updates in the emitted-but-not-persisted set, and its `PermissionDecided` is emitted straight from `Session::respond_permission` without a write. The reason is not volume: a request can only be answered by the child that asked, and no child survives a restart — so a persisted request would come back as a card whose buttons cannot work. Dropping it makes that stale card impossible rather than merely unlikely. Nothing is lost, because the tool call it belongs to *is* persisted and shows the outcome either way.
...
- **`requires_user_interaction` is parsed and ignored.** Set when the tool's own card is the interaction surface — `AskUserQuestion` and the elicitation-driven MCP prompts — meaning a bare Allow/Deny is not a real answer. This app renders Allow/Deny anyway, since it has no card of its own to show instead. Revisit alongside `request_user_dialog`, which is the other half of the same gap and is currently auto-refused.
