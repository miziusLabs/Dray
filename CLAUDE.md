...
- **The mapper is partial.** `assistant`, `user`, `result`, `system/init`, tasks, `background_tasks_changed`, and the stream frames are wired; `status`, hooks, `post_turn_summary`, `task_updated`, and `control_response` still fall through to `None`. `SystemEvent` has a `#[serde(other)]` catch-all, so an unknown subtype degrades instead of failing the line (`thinking_tokens` arrived unannounced and did exactly that). `turn_id` is always `None` — deliberately: Claude Code has no turn identifier on the wire, the UI groups a transcript by user message, and a promptless agent-opened `init` (see below) means minting our own ids would split what the reader sees as one exchange.
