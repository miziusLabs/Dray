...
/// The repository default branch, when the remote exposes one.
///
/// It reads `origin/HEAD` and falls back through `origin/main` then `origin/master`
...
/// Where this repository keeps its index. Git may use a per-checkout index,
/// and this reads the path Git reports rather than assuming `.git/index`.
///
...
pub async fn work_status(cwd: &str) -> WorkStatus {
    ...
    // name: a local `main` can be stale or absent entirely in a separate checkout, and
